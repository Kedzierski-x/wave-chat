...
const UserSearchDrawer = ({ isOpen, onClose, onAddFriend, }) => {
      ...
      <DrawerContent className="fixed left-0 top-0 w-full md:h-full lg:w-[500px] h-full bg-white shadow-md">
        ...
        <div className="p-4">
          ...
          <div className="flex flex-col gap-2">
            {searchResults.length > 0 ? (searchResults.map((user) => (<div key={user.id} className="p-2 border rounded cursor-pointer flex justify-between items-center gap-2">
                  <Avatar>
                ...
                </div>))) : (<p className="text-gray-500">No results found.</p>)}
          </div>
        </div>
      </DrawerContent>
    </Drawer>);
